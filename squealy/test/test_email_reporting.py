...
class  EmailReportTestCase(BaseTestCase):
    ...
    def test_email(self):
        ...
        self.chart.save()
        report = ScheduledReport(subject='Test subject 1',cron_expression = '*/2 * * * *', template=test_html)
        report.save()
        ReportRecipient(email='test1@test2.com',report=report).save()
        ScheduledReportChart(chart=self.chart,report=report).save()
        send_emails()
        ...
        self.assertEqual(content,mail.outbox[0].get_content())
